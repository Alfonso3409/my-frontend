...
const MyNavbar = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      //  need a way to show succesful signup
    } catch (error) {
      console.error("Login error:", error);
      // need to add a way to show an error message to the user
    }
  };
  ...
  const handleSignup = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:3002/api/user', { username: signupUsername, email: signupEmail, password: signupPassword });
      // need to show a success message, redirect, etc.
    } catch (error) {
      console.error("Signup error:", error);
      // need to show an error message to the user
    }
  };
  ...
  return (
    <Navbar bg="dark" variant="dark" expand="lg">
      <Container>
        <Navbar.Brand as={Link} to="/">The Store</Navbar.Brand>
